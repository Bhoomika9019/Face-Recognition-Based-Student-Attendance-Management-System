...
def save_pass():
    ...
    exists1 = os.path.isfile("TrainingImageLabel_sd.txt")
    if exists1:
        tf = open("TrainingImageLabel_sd.txt", "r")
        key = tf.read()
    else:
        master.destroy()
        ...
        if new_pas == None:
            ...
            tf = open("TrainingImageLabel_sd.txt", "w")
            tf.write(new_pas)
    ...
    if (op == key):
        if(newp == nnewp):
            txf = open("TrainingImageLabelsd.txt", "w")
            txf.write(newp)
        else:
...
def TakeImages():
    ...
    serial = 0
    exists = os.path.isfile("StudentDetails_studentDetails.csv")
    if exists:
        with open("StudentDetails_StudentDetails.csv", 'r') as csvFile1:
        ...
        with open("StudentDetails_StudentDetails.csv", 'a+') as csvFile1:
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        row = [serial, '', Id, '', name]
        with open('StudentDetails_StudentDetails.csv', 'a+') as csvFile:
...
def TrainImages():
    ...
    try:
        ...
        return
    recognizer.save("TrainingImageLabel_Trainner.yml")
...
def TrackImages():
    ...
    exists3 = os.path.isfile("TrainingImageLabel_Trainner.yml")
    if exists3:
        recognizer.read("TrainingImageLabel_Trainner.yml")
    else:
    ...
    exists1 = os.path.isfile("StudentDetails_StudentDetails.csv")
    if exists1:
        df = pd.read_csv("StudentDetails_StudentDetails.csv")
    else:
    ...
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
    exists = os.path.isfile("Attendance_Attendance_" + date + ".csv")
    if exists:
        with open("Attendance_Attendance_" + date + ".csv", 'a+') as csvFile1:
            writer = csv.writer(csvFile1)
        ...
        with open("Attendance_Attendance_" + date + ".csv", 'a+') as csvFile1:
            ...
            writer.writerow(attendance)
        csvFile1.close()
    with open("Attendance_Attendance_" + date + ".csv", 'r') as csvFile1:
        reader1 = csv.reader(csvFile1)
